...
fn process_onnx_nodes(
    ...
    for node in nodes {
        match node.op_type.as_str() {
            ...
            "Pow" => parse_pow_node(node, tensors, known_values)?,
            "Sqrt" => parse_sqrt_node(node, tensors, known_values)?,
            "Cos" => parse_cos_node(node, tensors, known_values)?,
            "Sin" => parse_sin_node(node, tensors, known_values)?,
            "Neg" => parse_neg_node(node, tensors, known_values)?,
            "Abs" => parse_abs_node(node, tensors, known_values)?,
            ...
            "Sigmoid" => parse_sigmoid_node(node, tensors, cx, weight_data)?,
            ...
            "LessOrEqual" => parse_less_or_equal_node(node, tensors, known_values)?,
